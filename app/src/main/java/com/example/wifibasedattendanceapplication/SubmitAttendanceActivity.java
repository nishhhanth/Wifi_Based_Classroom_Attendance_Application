...
public class SubmitAttendanceActivity extends BaseAuthenticatedActivity {
    ...
    private static final String KEY_NOTIFIED_PREFIX = "notified_session_";
    ...
    private void scanForEndedSessionsAndNotify(String studentDivision, @NonNull DataSnapshot allSessions) {
        ...
        for (DataSnapshot sessionSnapshot : allSessions.getChildren()) {
            ...
            if (candidateEnd >= latestEndTs) {
                latestEndTs = candidateEnd;
                latestEndedSessionId = sessionKey;
                latestSubject = sessionSnapshot.child("subject").getValue(String.class);
                latestPeriodDate = sessionSnapshot.child("period_date").getValue(String.class);
            }
        }

        if (latestEndedSessionId != null) {
            resolveFinalStatusAndNotify(latestEndedSessionId, latestSubject, latestPeriodDate);
        }
    }
    ...
    private void startRealTimeSessionMonitoring(String studentDivision) {
        ...
        attendanceReportRef.addChildEventListener(new ChildEventListener() {
            ...
            public void onChildChanged(@NonNull DataSnapshot sessionSnapshot, String previousChildName) {
                ...
                String periodDate = sessionSnapshot.child("period_date").getValue(String.class);
                ...
                if (studentDivision.equals(sessionDivision) &&
                    ...
                    new android.os.Handler().postDelayed(() -> {
                        resolveFinalStatusAndNotify(sessionKey, subject, periodDate);
                    }, 1500);
                }
            }
            ...
            @Override
            public void onCancelled(@NonNull DatabaseError error) {
                Log.e("SubmitAttendance", "ChildEventListener cancelled: " + error.getMessage());
            }
        });
    }

    private boolean hasNotified(@NonNull String sessionId) {
        SharedPreferences prefs = getSharedPreferences(PREFS_NAME, MODE_PRIVATE);
        return prefs.getBoolean(KEY_NOTIFIED_PREFIX + sessionId, false);
    }

    private void markNotified(@NonNull String sessionId) {
        getSharedPreferences(PREFS_NAME, MODE_PRIVATE)
                .edit()
                .putBoolean(KEY_NOTIFIED_PREFIX + sessionId, true)
                .apply();
    }

    private void resolveFinalStatusAndNotify(@NonNull String sessionId, String subject, String periodDate) {
        // Prefer student's Attendance node (P/A), fallback to session Students node
        DatabaseReference studentAttendance = FirebaseDatabase.getInstance()
                .getReference("Students").child(currentStudentEnrollment)
                .child("Attendance").child(sessionId);

        studentAttendance.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snap) {
                String code = snap.getValue(String.class); // P or A
                String display = mapAttendanceCode(code);
                if (display == null) {
                    // Fallback to session-level status text
                    DatabaseReference sessionStatusRef = FirebaseDatabase.getInstance()
                            .getReference("AttendanceReport").child(sessionId)
                            .child("Students").child(currentStudentEnrollment).child("attendance_status");
                    sessionStatusRef.addListenerForSingleValueEvent(new ValueEventListener() {
                        @Override
                        public void onDataChange(@NonNull DataSnapshot s2) {
                            String statusText = s2.getValue(String.class);
                            String finalDisplay = (statusText == null) ? "Not Marked" : statusText;
                            postAttendanceNotification(sessionId, subject, periodDate, finalDisplay);
                        }

                        @Override
                        public void onCancelled(@NonNull DatabaseError error) {
                            postAttendanceNotification(sessionId, subject, periodDate, "Not Marked");
                        }
                    });
                } else {
                    postAttendanceNotification(sessionId, subject, periodDate, display);
                }
            }
    ...
    private void postAttendanceNotification(@NonNull String sessionId, String subject, String periodDate, String displayStatus) {
        String title = "Attendance finalized: " + (subject == null ? "" : subject);
        String message = "Date: " + (periodDate == null ? "" : periodDate) + " | Status: " + displayStatus;
            ...
            if (hasLocationPermissions()) {
                ...
                if (scanResults != null && !scanResults.isEmpty()) {
                    ...
                    for (android.net.wifi.ScanResult result : scanResults) {
                        ...
                        if ("PU-Student_Wifi".equals(ssid)) {
                            Log.d("WiFiDebug", "Found "+ssid+" network in scan results");
                            return true;
                        }
                    }
